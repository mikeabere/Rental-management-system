...
const api=async(path,options={})=>{
  ...
  const data=await response.json().catch(()=>({})); if(!response.ok)throw new Error(data.message||'Request failed');
   return data;
  };
function Auth({onLogin}){
  ...
  const submit=async e=>{
    e.preventDefault();
    setError('')
    try{const data=await api(`/api/auth/${mode}`,
...
function App(){
  ...
  [error,setError]=useState(''); 
  useEffect(()=>{
    if(localStorage.token)api('/api/auth/me').then(d=>setUser(d.user)).catch(()=>localStorage.removeItem('token'))},[]); 
    useEffect(()=>{if(user){Promise.all([api('/api/dashboard/summary'),
                  ...
                  <main className="content">
                    <header>
                      <div>
                        ...
                        <div className="date">{new Date().toLocaleDateString('en-KE',{weekday:'long',day:'numeric',month:'long'})}
                          </div>
                          </header>
                                ...
                                <section className="stats">
                                  ...
                                  <section className="panel">
                                    <div className="panel-head"><div>
                                      ...
                                      {payments.length?<div className="table">{payments.slice(0,6).map(p=><div className="row" key={p._id}><div><strong>{p.mpesaReceiptNumber||'M-Pesa payment'}</strong><small>{new Date(p.createdAt).toLocaleDateString('en-KE')}</small></div><strong>KES {Number(p.amount).toLocaleString()}</strong><span className={`status ${p.status.toLowerCase()}`}>{p.status}</span></div>)}</div>:<div className="empty">No payment activity yet. M-Pesa payments will appear here after a callback is received.</div>}</section></main></div>}
